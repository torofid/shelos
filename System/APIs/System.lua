...
CheckUpdates = function(showWindow, bedrock)
	...
	if http then
		if showWindow then
			bedrock:DisplayAlertWindow("Update shelOS", "Checking for updates, this may take a moment.", {'Ok'})
		end
		http.request('https://api.github.com/repos/oeed/shelOS/releases#')
...
OnHTTPSuccess = function(bedrock, event, url, data)		
	...
	if url == 'https://api.github.com/repos/oeed/shelOS/releases#' then
		if not data then
		...
		if version == latestReleaseTag then
			...
			Log.i('shelOS is up to date.')
			if _showWindow then
				if requestBedrock.Window then
					requestBedrock.Window:Close()
				end
				requestBedrock:DisplayAlertWindow("Up to date!", "shelOS is up to date!", {'Ok'})
			end
			return
		elseif SematicVersionIsNewer(GetSematicVersion(latestReleaseTag), GetSematicVersion(version)) then			
			Log.i('New version of shelOS available. (from '..version..' to '..latestReleaseTag..')')
			if requestBedrock.Window then
				requestBedrock.Window:Close()
			end
			requestBedrock:DisplayAlertWindow("Update shelOS", "There is a new version of shelOS available, do you want to update?", {'Yes', 'No'}, function(value)
				if value == 'Yes' then
					SetBootArgs('update')
					if requestBedrock.Window then
						requestBedrock.Window:Close()
					end
					requestBedrock:DisplayAlertWindow("Update shelOS", "To finish updating reboot your computer.", {'Ok'})
				end
			end)
		else
			Log.i('shelOS is neither up to date or behind. (.version probably edited)')
		end
	end
end
