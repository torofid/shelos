...
--
--		Bedrock is the core program framework used by all shelOS and OneCode programs.
...
LoadImage = function(path, global)
	...
	if fs.exists(path) then
		local _io = io
		if shelOS and global then
			_io = shelOS.IO
		end
        local file = _io.open(path, "r")
        if not file then
        	error('Error Occured. _io:'..tostring(_io)..' shelOS: '..tostring(shelOS)..' shelOS.IO'..tostring(shelOS.IO)..' io: '..tostring(io))
        end
...
DrawBuffer = function()
	...
	-- If the program is within shelOS pass our buffer straight to the OS to draw rather than fidling around with the term API

	if shelOS and shelOS.Buffer then
		Drawing.Buffer = shelOS.Buffer
	else
...
TidyPath = function(path)
	path = '/'..path
	if fs.exists(path) and fs.isDir(path) or (shelOS and shelOS.FS.exists(path) and shelOS.FS.isDir(path)) then
		path = path .. '/'
	end
...
GetInfo = function(p)
	...
	elseif p.Type == 'monitor' then
		...
		if shelOS.Settings:GetValues()['Monitor'] == p.Side:lower() then
				...
				self.Bedrock:DisplayAlertWindow('Reboot Required', "To change screen you'll need to reboot your computer.", {'Reboot', 'Cancel'}, function(value)
					if value == 'Reboot' then
						if buttonTitle == 'Use Computer Screen' then
							shelOS.Settings:SetValue('Monitor', nil)
						else
							shelOS.Settings:SetValue('Monitor', p.Side:lower())
						end
						shelOS.Reboot()
					end
				end)
...
function LoadView(self, name, draw)
	...
	if h then
		...
		if view then
			...
			if shelOS and view.ToolBarColour then
				shelOS.ToolBarColour = view.ToolBarColour
			end
			if shelOS and view.ToolBarTextColour then
				shelOS.ToolBarTextColour = view.ToolBarTextColour
			end
...
local function isFolder(path)
	...
	local extension
	if shelOS and shelOS.System then
		extension = shelOS.System.RealExtension(path)
